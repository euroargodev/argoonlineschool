...
## A unique identifiers for each float: the WMO 
Each one of the Argo floats have a unique indentifier, kwnon as the World Meteorological Organization (WMO).
...
Since the target is to have the Argo data available within 24 hours of their transmission from the float, this data is called *Real Time* (RT) data.  One year after the data were observed it is, if necessary, adjusted and made public, this data is called *Delayed Mode* (DM). 

*In the next section we describe to access the RT data generated by one float.*
