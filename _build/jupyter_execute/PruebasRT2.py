...
for iwmo in [1900379,
    ...
    psal1=prof.PSAL.where(prof.PSAL_QC.values.astype(float) == 1)
    temp1=prof.TEMP.where(prof.PSAL_QC.values.astype(float) == 1)
    psal4=prof.PSAL.where(  (prof.PSAL_QC.values.astype(float) == 3) 
                          | (prof.PSAL_QC.values.astype(float) == 4)
                          | (prof.PSAL_QC.values.astype(float) == 5))
    temp4=prof.TEMP.where(  (prof.PSAL_QC.values.astype(float) == 3) 
                          | (prof.PSAL_QC.values.astype(float) == 4)
                          | (prof.PSAL_QC.values.astype(float) == 5))

    ax.plot(psal1,temp1,'bo')
    ax.plot(psal4,temp4,'ro')
    ax.set_title(f"{iwmo}")
    ax.grid()
